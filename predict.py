...
def predidct_age_gender_race(save_prediction_at, imgs_path = 'cropped_faces/'):
    ...
    model_fair_7_hm = HookedModel(model_fair_7)
    model_fair_7_act=GetActivationsHook('avgpool')
    model_fair_7_hm.apply_hook('avgpool',model_fair_7_act)

    activations = pd.DataFrame([],columns=['image_paths']+[i for i in range(512)])
    ...
    for index, img_name in enumerate(img_names):
        ...
        with torch.no_grad():
            model_fair_7_hm(image)
            activations.loc[len(activations)] = [img_name]+torch.squeeze(model_fair_7_act.get_activations()).tolist()
    ...
    activations.to_csv('fairface_activations.csv') 
...
if __name__ == "__main__":
    ...
    SAVE_DETECTED_AT = "detected_faces_india"
    ...
    predidct_age_gender_race("test_outputs_cfd_india.csv", SAVE_DETECTED_AT)
